...
(() => {
  ...
  function upsellCats() {
    // Use server-filtered upsell_products — only admin-allowed categories.
    const up = state.menu?.upsell_products;
    if (!up || !Object.keys(up).length) return [];
    return Object.keys(up).filter(c => up[c]?.some(p => p.in_stock !== 0));
  }
  ...
  function renderUpsellProducts(cat) {
    ...
    // Use upsell_products (server-filtered) not products (full menu)
    const up = state.menu?.upsell_products || {};
    const products = (up[cat] || []).filter(p => p.in_stock !== 0);
    list.innerHTML = "";
    list.scrollLeft = 0;
  ...
  async function openCart() {
    ...
    renderCartItems();
    // Re-fetch upsell_products from server every time cart opens so
    // admin changes take effect immediately without a full page reload.
    try {
      const r = await fetch(`/api/menu?tenant=${encodeURIComponent(tenantId)}`);
      if (r.ok) {
        const fresh = await r.json();
        if (state.menu) {
          state.menu.upsell_products = fresh.upsell_products;
          state.menu.upsell_categories = fresh.upsell_categories;
        }
      }
    } catch (_) {}
    renderCartUpsells();
